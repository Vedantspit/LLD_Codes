...
public class SubscriptionRepositoryImpl implements SubscriptionRepository {
    ...
    public List<Subscription> findBySubscriber(String subscriberId) {
        return subscriptions.values().stream().filter(sub -> sub.getTopicId().equals(subscriberId) && sub.isActive())
                .collect(Collectors.toList());
    }

    @Override
    public List<Subscription> findByTopic(String topicId) {
        return subscriptions.values().stream().filter(sub -> sub.getTopicId().equals(topicId) && sub.isActive())
                .collect(Collectors.toList());
    }
